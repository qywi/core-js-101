...
function getCommonDirectoryPath(pathes) {
  ...
  const isCommonSegment = (segment, index) => splitPaths.every((path) => path[index] === segment);
  const commonPath = splitPaths[0].filter(isCommonSegment);
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m1.length; i += 1) {
    ...
    for (let j = 0; j < m2[0].length; j += 1) {
      result[i][j] = m1[i].reduce((sum, elm, idx) => sum + elm * m2[idx][j], 0);
    }
  }
  return result;
}
